...
class ReverseAlphabeticalComparatorTest {
	...
	void checkReverseComparatorChildren() {
		...
		assertArrayEquals(expected,elementToStringArray(root.getChildren(new ReverseAlphabeticalComparator())));
	}
	...
	void checkReverseComparatorDirc() {
		...
		for(ApfsElement e : root.getSubDirectories(new ReverseAlphabeticalComparator())){
			actual.add(e);
		}
	...
	void checkReverseComparatorFiles() {
		...
		LinkedList<ApfsElement> actual = new LinkedList<ApfsElement>();
		for(ApfsElement e : root.findDirByName("system").getFiles(new ReverseAlphabeticalComparator())){
			actual.add(e);
		}
		assertArrayEquals(expected,elementToStringArray(actual));
	}
